function output = advectMOD(method, N, tau, nStep)      %MOD
% method: 1 = FTCS, 2 = Lax, 3 = Lax-Wendroff           %MOD
% N: number of grid points                              %MOD
% tau: time step                                        %MOD
% nStep: number of steps                                %MOD
%% advectMOD - Program to solve the advection equation  %MOD
% using the various hyperbolic PDE schemes
% invoke as (for example): advectMOD(1, 100, 0.001, 200)%MOD
% Original by AJG; modified by Katrina Carver 20200406  %MOD
help advectMOD;  % Clear memory and print header        %MOD

%% * Select numerical parameters (time step, grid spacing, etc.).
%menu('Choose a numerical method:', ...     %MOD
     %  'FTCS','Lax','Lax-Wendroff');       %MOD
%N = input('Enter number of grid points: ');%MOD
L = 1.;     % System size
h = L/N;    % Grid spacing
c = 1;      % Wave speed
