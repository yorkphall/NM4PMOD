function dummy_output = pendulMOD1(NumericalMethod,theta0,tau,nstep)	%MOD
% pendulMOD1 - Program to compute the motion of a simple pendulum  %MOD
% using the Euler or Verlet method  %MOD
% invoke for example as:  pendulMOD1(1,45,0.01,3000)		%MOD
% NumericalMethod is the method to use for the calculation. %MOD
% For Euler, NumericalMethod = 1, %MOD
% For Verlet, NumericalMethod = 2, %MOD
% theta0 is the initial angle of the pendulum in degrees  %MOD
% tau is the time step  %MOD
% nstep is the total number of steps  %MOD
% Original by AJG; modified by Alexander Koucoulas 20200406 %MOD
help pendulMOD1;	% Print header %MOD
%clear all;  help pendul;   % Clear memory and print header %MOD
...
for istep=1:nstep  
  ...
  else if( NumericalMethod == 2 ) %MOD
          theta_new = 2*theta - theta_old + tau^2*accel;
          theta_old = theta;			   % Verlet method
          theta = theta_new;
      else
          fprintf('Invalid argument for NumericalMethod entered. Rerun with either a 1 for Euler or 2 for Verlet \n') %MOD
          break;
      end
  end 
